...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.film = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(dim, 2 * dim, kernel_size=1, bias=True)  # -> [gamma, beta]
        )
    ...
    def forward(self, x, noise_embed=None):
        ...
        if noise_embed is not None:
            gamma_beta = self.film(noise_embed)                 # [B, 2C, H, W]
            gamma, beta = gamma_beta.chunk(2, dim=1)            # [B, C, H, W] each
            y = y * (1 + gamma) + beta                          # FiLM modulate
